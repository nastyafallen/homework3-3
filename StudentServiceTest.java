...
class StudentServiceTest {
    ...
    @Test
    void getStudent() {
        Mockito.when(repositoryMock.findById(0L)).thenReturn(Optional.of(student1));
    ...
    void updateStudent() {
        Mockito.when(repositoryMock.save(student1)).thenReturn(student1);
        Mockito.when(repositoryMock.findById(1L)).thenReturn(Optional.of(student1));
        out.createStudent(student1);
        Student student2 = new Student(4L, "test2", 34);
        out.updateStudent(1, student2);
        assertThat(out.getStudent(1).get().getName()).isEqualTo("test2");
        assertThat(out.getStudent(1).get().getAge()).isEqualTo(34);
    }
